...
def sync(src,dest,verbose=False,quiet=False):
    for f in os.listdir(src):   # every file and directory in the path 'src'
        ...
        if os.path.isdir(pathname): # if the current item is a directory
            ...
            sync(pathname,pathname2,verbose,quiet)    # recursively sync data in directory
        elif os.path.isfile(pathname):  # if the current item is a file
            if verbose and not quiet:
                print(pathname, " is a file")
            copyto(pathname,pathname2,verbose,quiet)  # copy to destination
        else:
            print('Error: skipping ' , pathname)   # else print error
    

def copyto(src,dest,verbose=False,quiet=False):
...
sync(args.source,args.dest,args.verbose,args.quiet)
